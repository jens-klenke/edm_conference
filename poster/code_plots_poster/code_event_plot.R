...
plot_cluster_stud <- function(data, point_data, clust, k) {
  ...
  time_plot <- data %>%
    ...
    theme(
      ...
      axis.title = element_text(size = 20),### from 8
      legend.title = element_text(size = 16)) + ### from 8
  ...
  point_plot <- point_data %>%
    ...
    theme(
      ...
      axis.title.y = element_text(size = 20),###from 8
      legend.title = element_text(size = 16)) + ### from 8
    scale_fill_manual(
...
#grid_plot_list(plot_list, 1:6)
#ggsave(filename = "04_plots/Student_comparison/cowplot_comp_6.png", 
#       height = 9, width = 18)

changed_order <- c(1,3,5,2,4,6)

grid_plot_list(plot_list, changed_order)
