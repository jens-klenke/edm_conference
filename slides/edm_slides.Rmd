...
* In this case take-home exams are conducted as open-book, but collaboration is strictly prohibited
...
Categorize students with hierarchical clustering algorithm on event logs of our statistical exam and strengthen the analysis with a comparison to a proctored exam group
]
...
* Both exams are conducted digital with the e-assessment system [JACK](https://www.uni-due.de/zim/services/jack.php) 
...
* The difference between the two courses is marginal
...
.font80[
* We compare two different kinds of attributes
  * Dissimilarities in the student´s event patterns (time of submission)
...
Dissimilarities in the student´s event patterns (time of submission) for each task $j$  

$$d_j^L(v_{ij}, v_{i'j}) = \sum_{m=1}^{K=70} | v_{ijm} - v_{i'jm} |$$


* With 
  * Examination is devided into $m = 1, ... , 70$ time intervals
...
* We reduce the weights for
  * `R`-tasks as these task have more noice
  * Free-text questions as the comparison on that kind of task is limited
  * Points achieved
...
```{r, out.width="100%", fig.align="center", fig.cap=c("<strong> Figure 1: </strong> Dendogram produced by average linkage clustering of the proctored control group (2018/19). <strong> G-L </strong> mark the clusters with the lowest dissimilarity"), echo=FALSE}
...
* The approach provides a basis for examination of clusters based on comparison with a reference group
  * However, the ground truth is not known, limiting the certainty of conclusions
